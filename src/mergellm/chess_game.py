...
class ChessGame:
    def __init__(self, white_player='llm', round=6):
        ...
        self.rounds = round
        if white_player == "llm":
            self.llm_convo = LLMHuggingfaceConversation(self.gpt3_chat, self.huggingface, self.stockfish, max_rounds=self.rounds)
            ...
            self.llm_convo = LLMHuggingfaceConversation(self.gpt3_chat, self.huggingface, self.stockfish, max_rounds=self.rounds, color="black")
    ...
    def play_game(self):
        ...
        while True:
            if self.white_player == 'llm':
                ...
                suggested_move, round = self.llm_convo.discuss_move(self.current_game_state)
...
if __name__ == "__main__":
    ...
    num_games = 10
    detailed_fieldnames = ['Game', 'Winner', 'Color', 'Number of Moves', 'Game Duration']
    summary_fieldnames = ['LLM Wins (White)', 'Stockfish Wins (Black)', 'LLM Wins (Black)', 'Stockfish Wins (White)']
    results = {field: 0 for field in summary_fieldnames}
    total_time = 0
    rounds = [5]
    for round_num in rounds:
        with open(f'detailed_chess_game_results_round_sf1.csv', 'w', newline='') as file_detailed, \
                open(f'chess_game_results_round_sf1.csv', 'w', newline='') as file_summary:
            

            detailed_writer = csv.DictWriter(file_detailed, fieldnames=detailed_fieldnames)
        ...
        for i in range(num_games):
            ...
            winner = 'LLM' if (result == '1-0' and color == 'White') or (result == '0-1' and color == 'Black') else 'Stockfish'
            winner = 'LLM' if (result == '1/2-1/2') else winner
            print(f"Winner: {winner}")

            results[f'{winner} Wins ({color})'] += 1

            with open(f'detailed_chess_game_results_round_sf1.csv', 'a', newline='') as file_detailed:
                detailed_writer = csv.DictWriter(file_detailed, fieldnames=detailed_fieldnames)
                game_detail = {
                    'Game': i + 1,
                    'Winner': winner,
            ...
            with open(f'chess_game_results_round_sf1.csv', 'a', newline='') as file_summary:
                summary_writer = csv.DictWriter(file_summary, fieldnames=summary_fieldnames)
